...
- **Scene / background reference images** (for static avatar stabilization): `/generate_reference` or `/upload_reference` creates a "baked" full avatar on the desired place/background. `/generate_reference` uses **neutral clear daytime conditions** (current season, `apply_holidays=False`) rather than live weather so the reference is stable and easy to adapt. When present, `async_update_avatar` uses it as the conditioning image, and `_prepare_prompt(..., include_place=False, apply_holidays=...)` controls substitution. Re-bake after major location changes. `reference.jpg` lives next to `avatar.jpg`. Migration 005 added the column. Cache uses the active reference bytes when provided.
